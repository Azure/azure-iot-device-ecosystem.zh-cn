...
-   用于访问命令行的 SSH 客户端，例如 [PuTTY](https://www.putty.org/)。
-   需要认证的硬件。
...
3.  使用以下说明创建设备并将其注册到 IoT 中心。
    ...
    b.保留“数据库类型”设置，即设置为“共享”。 列表中会显示已注册的设备。 如果该设备未显示在列表中，请单击“刷新”按钮。  如果这是第一次执行此操作，则不应检索任何信息。
...
组件可以预装，或者在分销点为客户安装。以下手动测试适用于支持通过包管理器（例如 dpkg 或 apt）完成更新，或者以原子映像形式完成更新的 Linux 分发版。
...
**说明：** 如果设备使用安全协议的自定义实现，而不是使用 Azure IoT 设备 SDK，则设备将认证为“级别 1”。 
...
**说明：** 如果设备可以使用 Azure IoT 设备 SDK 实现设备安全协议，则设备将认证为“级别 2”。 
...
**说明：** 如果设备使用硬件安全模块来增强安全性，则设备将认证为“级别 3”。  这种验证是通过安全认证完成的。
...
**说明：** 如果设备使用硬件安全模块来增强安全性，则设备将认证为“级别 4”。  这种验证是通过安全认证完成的。
 
**要求详细信息：** 此级别需要满足以下“安全性”认证要求：
...
1.  “设备管理手动测试”中所述的、由问题步骤记录器记录的 Zip 文件。
2.  手动测试 2 中所述的文本文件 
3.  装有 IoT Azure 运行时，并已寄送到 Microsoft 做进一步验证的设备。
